    ...
    export default function Board({ players = [], animatedPositions = {} }) {
      ...
      useEffect(() => {
        ...
        const computeSide = (rect) => {
          ...
          const availableHeight = Math.max(0, window.innerHeight - rect.top - mobileH - 12)
          return Math.floor(Math.min(rect.width, availableHeight))
        }
                  ...
                  return (
                    <>
                      ...
                      {ladderPaths.map(({ start, end }) => {
                        const source = cellCenters[start] ?? squareCenter(start, size)
                        const target = cellCenters[end] ?? squareCenter(end, size)
                        const localCellW = (size.width - (1 * 9)) / 10 || 11
                        const unitScaleLocal = (cellCenters[start]?.w ?? localCellW) / 11 || 1
